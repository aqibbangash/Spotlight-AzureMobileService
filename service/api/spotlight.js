exports.post = function(request, response) {
    ...
    // Get all request type text and user_id equals user_id
        requestTable.where({type : type, user_id : user_id}).read({
...
///////
            if(numAlready > 0){
                ...
                if(connectedWith != ""){
                    ...
                    // Get Online users
                    requestTable.where(function(u,abc,t){return this.user_id != u  && this.type == t && this.completed == false && (abc.indexOf(u) == -1);},user_id,abc,type).read({
                        success : function(requests){
                            if(requests.length > 0){
                                 requests.forEach(function(request){
                                            ...
                                            if(users.length > 0){
                                                ...
                                                function traverse(user){
                                                    ...
                                                    if(prefs.indexOf(user.gender) !== -1 && user.prefs.indexOf(userGender) !== -1){
                                                        ...
                                                        // Find Request
                                                        requestTable.where({user_id : user.id, type : type, completed : false, other_user : null}).read({
                                                            success : function(requests){
                                                                if(requests.length > 0){
                                                                    //console.log("if");
                                                                    // Update request
                                                                    ...
                                                                    requestTable.where(function(u,t){return this.user_id == u && this.other_user != null && this.other_user != '' && this.type == t && this.completed == true},user_id,type).read({
                                                                        success : function(requests){
                                                                            if(requests.length > 0){
                                                                                // Find User
